...
@st.cache_resource
def load_and_train():
    path = '/kaggle/input/datasets/organizations/uciml/pima-indians-diabetes-database/diabetes.csv'
    try:
        df = pd.read_csv(path)
    except:
        np.random.seed(42)
        ...
        df = pd.DataFrame({
            ...
            'SkinThickness': np.random.randint(10, 80, n),
            'Insulin': np.random.randint(20, 500, n),
            'BMI': np.random.uniform(18, 60, n).round(1),
            'DiabetesPedigreeFunction': np.random.uniform(0.07, 2.4, n).round(3),
            'Age': np.random.randint(21, 80, n),
            'Outcome': np.random.randint(0, 2, n)
        })

    cols_zeros = ['Glucose', 'BloodPressure', 'SkinThickness', 'Insulin', 'BMI']
    df[cols_zeros] = df[cols_zeros].replace(0, np.nan)
    df[cols_zeros] = df[cols_zeros].fillna(df[cols_zeros].median())
    ...
    for name, m in models.items():
        ...
        results[name] = {'acc': acc, 'report': report}

    return models, scaler, results, df

models, scaler, results, df = load_and_train()
